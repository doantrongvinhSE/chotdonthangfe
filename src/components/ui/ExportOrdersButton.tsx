...
export function ExportOrdersButton({ orders, className = '' }: ExportOrdersButtonProps) {
  ...
  const exportToExcel = () => {
    ...
    const fileName = `don_hang_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <button
      ...
      className={`flex items-center px-3 py-1.5 text-white text-sm font-medium rounded-lg transition-colors ${className}`}
      style={{
    ...
    >
      <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 48 48" className="mr-1.5">
